...
def spatial_coherence(ratemap, n_neighbours=8):
    '''
    Calculate spatial coherence as in [1]

    Returns Z-scored Pearsons R correlation between rate values of each bin
    and the mean of its n/2 neighbours on the left and n/2 neighbours
    on the right.

    Parameters
    ----------
    ratemap: np.ndarray
    width_boxcar: int, %2==0

    Returns
    -------
    out : float, z-scored pearsons r

    References
    ----------
    ..[1] Muller and Kubie, J. Neurosci. (1989)
          The firing of hippocampal place cells predicts the future
          position of freely moving rats
    '''
    assert type(n_neighbours) == int
    assert n_neighbours % 2 == 0
    # create normalized boxcar window
    boxcar_window = np.ones(n_neighbours+1, dtype=float)
    # do not include central value
    centr_idx = int(n_neighbours/2)
    boxcar_window[centr_idx] == 0
    boxcar_window /= np.sum(boxcar_window)
    # only take valid output for now. mirroring may be an alternative
    rm_convlvd = np.convolve(ratemap, boxcar_window, mode='valid')
    rm_sel = ratemap[centr_idx:-centr_idx]
    r, p = stats.pearsonr(rm_convlvd, rm_sel)
    z = np.arctanh(r)
    return z
