...
def plot_waveforms(sptr, color='r', fig=None, title='waveforms', lw=2, gs=None, fs=8):
    """
    ...
    if gs is None:
        ...
        for c in range(nrc):
            ...
            if sptr.left_sweep is not None:
                ...
                axs[c].axvspan(sptr.left_sweep, sptr.left_sweep, color='k',
                               ls='--')
            axs[c].set_xlabel(stime.dimensionality, fontsize=fs)
            axs[c].set_xticks([0, round(stime.max())])
            axs[c].set_xticklabels([0, np.round(stime.max().magnitude)], fontsize=fs-2)
            plt.setp(axs[c].get_yticklabels(), fontsize=fs-2)
            if c > 0:
                plt.setp(axs[c].get_yticklabels(), visible=False)
        axs[0].set_ylabel(r'amplitude $\pm$ std [%s]' % m.dimensionality, fontsize=fs)
        fig.suptitle(title)
    return fig
...
def plot_amp_clusters(sptrs, colors=None, fig=None, title=None, gs=None, fs=8):
    """
        ...
        for x in range(nrc - 1):
            for y in range(nrc - 1):
                if y <= x:
                    ...
                    ax.set_yticks([])
                    if x == nrc - 2:
                        ax.set_xlabel('ch %i' % (range(nrc)[y - 1]), fontsize=fs)
                    if y == 0:
                        ax.set_ylabel('ch %i' % (x), fontsize=fs)
        if colors is None:
            from matplotlib.pyplot import cm
            colors = cm.rainbow(np.linspace(0, 1, len(sptrs)))
        for idx, sptr in enumerate(sptrs):
