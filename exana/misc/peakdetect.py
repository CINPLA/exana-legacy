...
def _pad(fft_data, pad_len):
    ...
    fft_data : array_like
        the fft
    pad_len : int
        By how many times the time resolution should be increased by
...
def _n(x):
    ...
    ----------
    x : int
        the value, which `2**n` must surpass
...
def _peakdetect_parabola_fitter(raw_peaks, x_axis, y_axis, points):
    ...
    ----------
    raw_peaks : list
        A list of either the maxima or the minima peaks, as given
        by the peakdetect functions, with index used as x-axis
    x_axis : array
        A numpy array of all the x values
    y_axis : array
        A numpy array of all the y values
    points : int
        How many points around the peak should be used during curve
        fitting, must be odd.

    Returns
    -------
    out : list
        A list giving all the peaks and the fitted waveform, format:
...
def peakdetect(y_axis, x_axis = None, lookahead = 200, delta=0):
    ...
    -------
    out : list
        two lists [max_peaks, min_peaks] containing the positive and
...
def peakdetect_fft(y_axis, x_axis, pad_len = 20):
    ...
    pad_len : int
        By how many times the time resolution should be
...
def peakdetect_sine(y_axis, x_axis, points = 31, lock_frequency = False):
    ...
    points : int
        How many points around the peak should be used during curve
        fitting (default: 31)
    lock_frequency : float
        Specifies if the frequency argument of the model
...
def peakdetect_sine_locked(y_axis, x_axis, points = 31):
    ...
    Returns
    -------
    out : see the function `peakdetect_sine`
    """
...
def peakdetect_zero_crossing(y_axis, x_axis = None, window = 11):
    ...
    y_axis : list
        A list containing the signal over which to find peaks
    x_axis : list
        A x-axis whose values correspond to the y_axis list
...
def zero_crossings(y_axis, window_len = 11,
    ...
    ----------
    y_axis : list
        A list containing the signal over which to find zero-crossings
    window_len : int
        the dimension of the smoothing window; should be an odd
        integer (default: 11)
    window_f : str
    ...
    offset_corrected : bool
        Used for recursive calling to remove offset when needed

    Returns
    -------
    out : int
        the index for each zero-crossing
    """
    # smooth the curve
    length = len(y_axis)
...
def zero_crossings_sine_fit(y_axis, x_axis, fit_window = None, smooth_window = 11):
    ...
    fit_window : int
        Number of points around the approximate zero crossing that
    ...
    Returns
    -------
    out : A list containing the positions of all the zero crossings.
    """
    # check input data
    x_axis, y_axis = _datacheck_peakdetect(x_axis, y_axis)
    #get first guess
